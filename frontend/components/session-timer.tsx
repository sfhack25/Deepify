...
export function SessionTimer() {
  ...
  const workProgress = ((workMinutes * 60 - workSecondsLeft) / (workMinutes * 60)) * 100
  const breakProgress = ((breakMinutes * 60 - breakSecondsLeft) / (breakMinutes * 60)) * 100
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-card-foreground">{formatTime(workSecondsLeft)}</span>
        </div>
        <Progress value={workProgress} className="h-2" />
        <div className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            {Math.floor((workMinutes * 60 - workSecondsLeft) / 60)}:
            {((workMinutes * 60 - workSecondsLeft) % 60).toString().padStart(2, "0")} / {workMinutes}:00
          </div>
          <div className="flex space-x-2">
            ...
            >
              {isWorkRunning ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />}
            </Button>
            <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={resetWorkTimer}>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-card-foreground">{formatTime(breakSecondsLeft)}</span>
        </div>
        <Progress value={breakProgress} className="h-2" />
        <div className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            {Math.floor((breakMinutes * 60 - breakSecondsLeft) / 60)}:
            {((breakMinutes * 60 - breakSecondsLeft) % 60).toString().padStart(2, "0")} / {breakMinutes}:00
          </div>
          <div className="flex space-x-2">
            <Button
              variant="outline"
            ...
            >
              {isBreakRunning ? <Pause className="h-3 w-3" /> : <Play className="h-3 w-3" />}
            </Button>
            <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={resetBreakTimer}>
              <RefreshCw className="h-3 w-3" />
            </Button>
          </div>
        </div>
